...
## Asset Changes (Editor Scripts)

Game assets like prefabs, attack data, combo trees, and animation controllers are not edited by hand — they're generated by editor scripts in `Assets/Editor/`. When a task changes game data (new attacks, balance tweaks, different combo flow), the workflow is:

1. Update the relevant editor script with the new values
2. Re-run the script from the Unity menu bar
3. The script regenerates the asset files

This keeps the scripts as the source of truth and makes changes reproducible. See [Unity Editor Scripts](unity-editor-scripts.md) for the full list, dependency graph, and common workflow recipes (e.g., "new animation GIFs arrived — which scripts to re-run").
