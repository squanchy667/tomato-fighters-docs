...
2. Define 8 stat fields matching the shared stat framework:
   ...
   - `pressureRate` (PRS) — float, range 0.5-2.0
...
8. For Viper, include both `meleeAttack` and `rangedAttack` fields (split scaling: 0.6 melee / 1.8 ranged)
...
| HP | 200 | 100 | 50 | 80 |
| DEF | 25 | 8 | 5 | 10 |
| ATK | 0.7 | 2.0 | 0.5 | 0.6 (melee) / 1.8 (ranged) |
| SPD | 0.7 | 1.3 | 1.0 | 1.1 |
| MNA | 50 | 60 | 150 | 120 |
| MRG | 2 | 3 | 8 | 6 |
...
- **Viper split ATK** — Viper has both `meleeAttack` (0.6) and `rangedAttack` (1.8); consider whether to add both fields to the base class or use a Viper-specific override. Recommended: add an optional `rangedAttack` field to the base class (defaults to -1 meaning "use attack") so the stat framework stays unified
